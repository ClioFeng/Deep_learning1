...
class DeepPromptCLIP(nn.Module):
    ...
    def __init__(self, args, dataset, template="This is a photo of {}"):
        ...
        # Compute text features for prompts
        ...
        # Consider the shape required for the deep prompt to be compatible with the CLIP model
        self.deep_prompt = nn.Parameter(torch.zeros_like(text_features[0]))
    ...
    def forward(self, image):
        ...
        # Compute the image features using the CLIP model and add deep prompts
        image_features = self.custom_encode_image(image)

        # Normalize image features
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)

        # Compute similarity logits between image features and text features
        logits = image_features @ self.text_features.t()

        # Multiply similarity logits with the logit scale
    ...
    def custom_encode_image(self, x):
        ...
        for i, block in enumerate(image_encoder.transformer.resblocks):
            ...
            if i == self.injection_layer:
                x += self.deep_prompt.unsqueeze(1).expand_as(x)
